...
begin
  ...
  call pgxls.format_cell(xls, format_code=>pgxls.get_format_code_numeric(3, true), font_name=>pgxls.get_font_name('monospace'), font_bold=>true);  
  --------------------------------------------------------------------------------------------------------------------------------
  ...
  for row in 1..10 loop
    ...
    for col in 1..10 loop      
      ...
      if row=2 and col=2 then call pgxls.merge_cells(xls, 5/*column_count*/); call pgxls.format_cell(xls, fill_foreground_color=>'light_red');   end if;
      if row=4 and col=4 then call pgxls.merge_cells(xls, row_count=>5);      call pgxls.format_cell(xls, fill_foreground_color=>'light_green'); end if;
    end loop;    
    if row=6 then call pgxls.merge_cells(xls, 4, 4, 6); call pgxls.format_cell(xls, fill_foreground_color=>'light_blue', column_=>6); end if;
  end loop;
  --------------------------------------------------------------------------------------------------------------------------------
